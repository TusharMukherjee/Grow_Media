...
const Navbar: React.FC = () => {
  ...
  return(
    <>
      <div className='col-span-7 flex justify-center h-12 items-center sticky left-0 right-0 top-0 sm:sticky sm:left-0 sm:right-0 sm:top-0 z-[70]  bg-teal-600'>
        <div className=' flex justify-center items-center mx-auto grow'>
            <div className=' flex flex-row justify-between mx-5 w-full'>
              ...
              <div className= {`${toggleSearch? 'visible': 'hidden'} flex flex-row lg:flex lg:flex-row border-[1px] border-teal-900 `}>
                  <input value={searchquery} onKeyDownCapture={(e) => e.key === 'Enter' && searchandremove()} onChange={(e)=>setsearchquery(e.target.value)} type="text" placeholder='Search' className=' text-white bg-teal-800 pl-3 outline-0' required/>
      ...
      <div className=' sticky left-0 top-12 right-0 status text-center bg-red-600 text-white'></div>
