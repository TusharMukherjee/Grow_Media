...
const Login = () => {
    ...
    <main className='grid grid-rows-6 grid-cols-7 h-screen w-screen' >
        ...
        <section className=' row-span-6 col-span-7 lg:col-span-4 lg:row-span-6 w-screen lg:w-full h-full grid place-content-center'>
            <div className=' lg:hidden z-0 fixed top-0 w-screen h-1/2 rounded-b-full lg:rounded-none col-span-3 bg-teal-500 lg:bg-white grid place-content-center '>
            </div>
            <div className=' bg-white border-[0.25px] h-auto w-auto border-gray-300 rounded-md shadow-md z-10'>
                <div className=' grid place-content-center py-5 sm:gap-4 gap-1'>
                    <h1 className=' font-light text-teal-500 sm:text-4xl text-2xl lg:text-2xl text-center sm:mb-8 lg:mb-6 mb-6 '>Log In</h1>
                    <div className=" my-5 lg:my-2">
                        <input onKeyDownCapture={(e) => e.key === 'Enter' && callLogin()} type="text" onChange={(e) => setUser((e.target.value).trim())} placeholder='Name' className=' bg-gray-200 text-gray-900 text-sm sm:text-lg lg:text-base sm:h-12 lg:h-10 h-8 sm:w-80 w-60 lg:w-60 sm:mx-10 lg:mx-6 mx-6 rounded-md outline-0 p-2' required/>
                    </div>
                    <div className=" my-5 lg:my-2">
                        <input onKeyDownCapture={(e) => e.key === 'Enter' && callLogin()} type="password" onChange={(e)=> setLoginPassword((e.target.value))} placeholder='Password' className=' bg-gray-200 text-gray-900 text-sm sm:text-lg lg:text-base sm:h-12 lg:h-10 h-8 sm:w-80 w-60 lg:w-60 sm:mx-10 lg:mx-6 mx-6 rounded-md outline-0 p-2' required/>
                    </div>
                    ...
                    {(isLoading)?(<><p className=' text-teal-600 mt-5 w-full text-center sm:text-lg lg:text-sm text-sm' >Logging In...</p></>):(<></>)}
                    {(errorPass)?<><p className=' text-red-600 mt-5 w-full text-center sm:text-lg lg:text-sm text-sm'>Password is incorrect</p></>:(<></>)}
                    {(errorUser)?<><p className=' text-red-600 mt-5 w-full text-center sm:text-lg lg:text-sm text-sm'>User is incorrect</p></>:(<></>)}
                    {(emptyField)?<><p className=' text-red-600 mt-5 w-full text-center sm:text-lg lg:text-sm text-sm'>Field can't be empty</p></>:(<></>)}
                </div>
            </div>
        </section>
    </main>
